...
Resolution order (first match wins):
  ...
  3. ~/.agentfile/.api-secret           — machine secret written by the API on startup;
                                          only used when the API URL is localhost
"""
...
def read_token(api_url: str) -> str | None:
    ...
    # 3. Machine secret — only for localhost (same machine as API)
    try:
        host = urllib.parse.urlparse(api_url).hostname or ""
    except Exception:
        host = ""
    if host in ("localhost", "127.0.0.1") and SECRET_FILE.exists():
